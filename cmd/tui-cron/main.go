// Command tui-cron is a terminal UI for everything scheduled on a machine:
// systemd timers and cron, on one list, with the schedule written out in
// English next to the expression that produced it. It previews the exact
// command line of every change before running it. systemd and cron are the two
// backends implemented today; the code is written against a generic interface
// so another scheduler could follow.
package main
...
// /etc/tui-cron/config.toml and ~/.config/tui-cron/config.toml.
const toolName = "tui-cron"

// systemdBackend is the manifest's name for the scheduler whose version gates a
// read path. cron's block is declared too, and carries no version command at
// all — see cmd/tui-cron/compat.go.
const systemdBackend = "systemd"

// version is stamped by the release build (-ldflags "-X main.version=…").
var version = "dev"

// defaults declares the configuration keys tui-cron understands. Only these are
// read from the environment (TUI_CRON_SUDO, …).
...
func parseFlags(args []string, out *os.File) (options, error) {
	...
	fs.BoolVar(&opts.check, "check", false,
		"read the schedulers and print the parsed state as JSON, then exit "+
			"(no UI, no changes); exit 1 if neither backend can be read")
	...
	fs.Usage = func() {
		_, _ = fmt.Fprintf(out, "tui-cron — every scheduled job on the machine, "+
			"systemd timers and cron together\n\n"+
			"Usage:\n  tui-cron [flags]\n\nFlags:\n")
		fs.PrintDefaults()
		_, _ = fmt.Fprintf(out, "\nConfiguration is read from %s, then %s, "+
			"then TUI_CRON_* in the environment.\n",
...
func run(args []string) error {
	...
	// no schedule and needs no privileges, and it survives a machine with
	// neither scheduler, because "there is nothing here to drive" is one of the
	// things a bug report has to be able to say. So it comes before the backend
	// is required.
	if opts.report {
		return runReport(cfg, opts, os.Stdout)
	}

	// The scheduler versions are probed once, before the backend is built,
	// because the backend needs the capability set: whether `systemctl
	// list-timers` can answer in JSON is a version question, and the answer
	// comes from the manifest.
	backendCompat := probeCompat(context.Background(), opts.demo)

	backend, err := pickBackend(cfg, opts, capsFor(backendCompat, systemdBackend))
	if err != nil {
		return err
	}

	// --check is the non-interactive path: it reads the schedulers and prints,
...
func pickBackend(cfg config.Config, opts options,
	caps compat.Caps) (schedule.Backend, error) {
	if opts.demo {
		return jobs.NewFake(), nil
	}
	return jobs.NewReal(cfg.SudoPrefix(), caps)
}
