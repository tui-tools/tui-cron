...
// enough to place an `OnCalendar` line under the section header without
// turning a ten line file into a wall of text in the confirm dialog.
const diffContext = 2
...
// "everything out, everything in", because the confirm dialog for a schedule
// change has one job: show the line that changed. A diff that repeats the whole
// crontab every time a line moved buries exactly that.
//
// The files here are a crontab or a drop-in, so the quadratic table costs
// nothing.
